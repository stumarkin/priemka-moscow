          ...
          <div class="row" id="app">
              <div class="col px-3">
                ...
                <div v-if="!isPayed" class="p-4 rounded-3" style=" background-image: url('https://priemka-pro.ru/webview/pro/img/pro_landing_form.png'); background-position: right top; background-repeat: no-repeat; background-color:  #e6edfb;">
                  ...
                  <div>
                      <a href="https://t.me/stumarkin" target="_blank" class="btn btn-primary btn-lg w-100 mt-4" >
                          Написать в Telegram
                      </a>
                  </div>
                </div>
            ...
            const app = createApp({
                data() {
                    return {
                      faq: [
                        {
                          q: 'На объекте несколько комнат и санузлов. Как их добавить в приёмку?',
                          a: `<p>После нажатия кнопки <i>Начать приемку</i> по умолчанию помещения не добався. Вы можете добавить их вручную в соответствии с вашим планом следования по помещениям на объекте. Для это используйте кнопку "Добавить помещение" и выберете один или несколько пунктов. Если нужно добавить несколько однотипных помещений, сделайте это последовательно несколько раз. Помещения всегда добавляются в конец списка.</p>
                        ...
                        },
                        {
                          ...
                          a: `<p>Набор проверок - это несколько пунктов проверок определенного типа строительных/отделочных работ в помещении. Структура и содержание проверок и соответствие их помещениям и типу отделкти на объекте настраивается в <a href="https://my.priemka.msk.ru/">панели управления</a>.</p>
                        ...
                        },
                        {
                          ...
                          a: 'На экране проверки помещения под списком наборов проверок есть текстовое поле, в которое можно добавить дополнительные замечания в свободной форме. Такое же поля есть на экране проверки. Замечания в свободной форме добавляются в в акт осмотра и в заключение.'
                        },
                        {
                          q: 'Как получить список выявленных недостатков?',
                          a: 'На экране приемки под списком помещений расположены кнопки получения списка недостатков, акта осмотра и заключения. Далее и то и другое вы можете отправить (поделиться) через любой мессенджер.'
                        },
                      ],
